...
with ui.row():
    with ui.card():
        ...
        with ui.row():
            ui.label("        ")
            ui.button('<', on_click=fn.moverelneg_1).style("width:40px;height:40px;")
            ui.button('>', on_click=fn.moverelpos_1).style("width:40px;height:40px;")
            ui.label("        ")
            ui.label("        ")
            ui.label("        ")
            ui.button("ABS", on_click=fn.move_stage).style("width:40px;height:40px;")

        with ui.row():
            ui.button("HOME", on_click=fn.homeLaComanda).style("width:120px;height:40px;")
            ui.button("STOP", on_click=fn.trigger_e_stop).style("width:120px;height:40px;")
    ...
    with ui.card():
        ...
        with ui.row():
            ...
            ui.button("SET X", on_click=lambda: fn.set_piezo_1('X', mode_toggle.value)).style("width:70px;")

        with ui.row():
            ui.label("Y")
            ui.number(label='Y Target', value=0.0, format='%.2f').bind_value(fn.state, 'pzt_y_val')
            ui.button("SET Y", on_click=lambda: fn.set_piezo_1('Y', mode_toggle.value)).style("width:70px;")

        with ui.row():
            ui.label("Z")
            ui.number(label='Z Target', value=0.0, format='%.2f').bind_value(fn.state, 'pzt_z_val')
            ui.button("SET Z", on_click=lambda: fn.set_piezo_1('Z', mode_toggle.value)).style("width:70px;")

        with ui.row():
            ui.button("ZERO ALL", on_click=lambda: fn.zero_piezo_all_1(mode_toggle.value)).style("width:110px; height:40px;").props('color=warning')
            ui.button("SET ALL", on_click=lambda: fn.set_piezo_all_1(mode_toggle.value)).style("width:110px; height:40px;")
